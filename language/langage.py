...
def p_expression_ajouter(p):
    'expression : AJOUTER PERSONNE NOM EQUALS noms PRENOM EQUALS prenoms LOGIN EQUALS ID MDPASS EQUALS TEXT SEXE EQUALS SEX TYPE EQUALS ID facul'
#    print(set(listeNom))
#    print(set(listePrenom))
    global glob_str
    global iddddddd
    ...
    if (p5!=None and p5!="") and (p8!=None and p8!=""):
        addPersonne(iddddddd,p5,p8,p[11],p[14],p17,p20)
        glob_str="Vous avez ajoute la personne "+p5 +" "+p8
    listeNom=[]
    listePrenom=[]
...
def p_expression_ajouter_module(p):
    ...
    global glob_str
    global iddddddd
    if (p[3]!=None and p[3]!="") and (p[6]!=None and p[6]!=""):
        addModule(iddddddd,p[3],p[6])
        glob_str="Vous avez ajoute le module "+ p[3] +" dans l'UV" + p[6]
