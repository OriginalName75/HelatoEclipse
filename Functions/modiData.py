...
def modModule(idP, p, nom=None, uv=None, groupes=None):
    ...
    if (nom != None and nom != "" and c.nom != nom) or (uv != None and c.theuv != uv) or (groupes != None):
        ...
        if nom != None and nom != "" and c.nom != nom:
            ...
            txt = txt + ". "
        if uv != None and uv != "" and c.theuv != uv:
            txt = txt + " Le module n\'est plus dans " + c.theuv.nom + " mais dans " + uv.nom + ". "
            cm2 = models.ChampsModifie()
            cm2.champs = mod
            cm2.nomchamp = 'UV changé depuis'
            cm2.valchamp = c.theuv
            cm2.save()
            c.theuv = uv
        if groupes != None:
...
def modNote(idP, p, note=None, personne=None, module=None, prof=None):
    ...
    if (note != None and note != "" and c.lanote != note) or (personne != c.personnenote and personne != "" and personne != None) or (module != c.themodule and module != "" and module != None) or (prof != None and prof != "" and c.prof != prof):
        ...
        if personne != None and personne != "" and c.personnenote != personne:
            txt = txt + " qui n\'est plus celle de " + c.personnenote.filter + " mais celle de " + personne.filter + ". "
            txt2 = ""
            txt3 = c.personnenote
            cm1 = models.ChampsModifie()
            cm1.champs = mod
            cm1.nomchamp = 'élèves changés depuis'
            cm1.valchamp = c.personnenote
            cm1.save()
            c.personnenote = personne
        else:
            txt2 = "Une de votre note a été modifié."
            txt = txt + " de " + personne.filter + ". "
        if note != None and note != "" and c.lanote != note:
            stri = "La note n\'est plus de " + str(c.lanote) + " mais de " + str(note) + ". "
            txt = txt + stri
            txt2 = txt2 + stri
            cm2 = models.ChampsModifie()
            cm2.champs = mod
            cm2.nomchamp = 'note changée depuis'
            cm2.valchamp = c.lanote
            cm2.save()
            c.lanote = note
        if module != None and module != "" and c.themodule != module:
            stri="Le module noté n\'est plus celui de " + str(c.themodule) + " mais de " + str(module) + ". "
            txt2 = txt2 + stri
            txt = txt + stri
            cm3 = models.ChampsModifie()
            cm3.champs = mod
            cm3.nomchamp = 'modules changés depuis'
            cm3.valchamp = c.themodule
            cm3.save()
            c.themodule = module
            
        if prof != None and prof != "" and c.prof != prof:
        ...
        if txt3==None:
            ...
            n3 = models.News()
            n3.txt = "Vous avez une nouvelle note : " + str(c.lanote) + " au module " + str(c.module)
            n3.typeG = MODIFIER
            n3.type = SALLESTATUT
...
def modSalle(idP, p, nom=None, capacite=None, typee=None):
    ...
    if (nom != None and nom != "" and  c.nom != nom) or (capacite != None and capacite != "" and capacite != c.capacite) or (typee != None and int(typee) != int(c.type)) :
        mod = models.Modification()
        mod.datemodif = timezone.now()
        ...
        if nom != None and nom != "" and c.nom != nom:
            ...
            cm1.save()
            c.nom = nom
        if capacite != None and capacite != "" and capacite != c.capacite:
            if i == 1:
                txt = txt + " et sa "
            else:
                txt = txt + "la "
